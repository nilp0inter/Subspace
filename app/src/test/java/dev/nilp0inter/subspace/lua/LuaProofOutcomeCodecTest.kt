...
class LuaProofOutcomeCodecTest {
    @Test
    fun `decodes every normalized outcome kind with its native evidence`() {
        val cases = listOf(
            DecodeCase(
                name = "created provenance",
                json = """{"kind":"created","stateId":41,"generation":3,"luaVersion":"Lua 5.4.8","bindingVersion":"mlua 0.12.0","topology":"jvm_owned"}""",
                expected = LuaProofOutcome.Created(
                    stateId = 41,
                    generation = 3,
                    luaVersion = "Lua 5.4.8",
                    bindingVersion = "mlua 0.12.0",
                    topology = "jvm_owned",
                ),
            ),
            DecodeCase(
                name = "completed coroutine result and latency",
                json = """{"kind":"completed","stateId":41,"generation":3,"coroutineId":17,"value":"result","elapsedNanos":9123,"currentBytes":128,"peakBytes":512,"deniedAllocations":1,"bridgeBytes":32,"luaVersion":"Lua 5.4.8","bindingVersion":"mlua 0.12.0","topology":"jvm_owned"}""",
                expected = LuaProofOutcome.Completed(41, 3, 17, "result", 9123, 128, 512, 1, 32, "Lua 5.4.8", "mlua 0.12.0", "jvm_owned"),
            ),
            DecodeCase(
                name = "yielded operation owner",
                json = """{"kind":"yielded","stateId":41,"generation":3,"coroutineId":17,"operationId":29,"value":"network"}""",
                expected = LuaProofOutcome.Yielded(41, 3, 17, 29, "network"),
            ),
            DecodeCase(
                name = "syntax failure diagnostic",
                json = """{"kind":"syntax_failure","stateId":41,"generation":3,"diagnostic":"line 7: unexpected end"}""",
                expected = LuaProofOutcome.SyntaxFailure(41, 3, "line 7: unexpected end"),
            ),
            DecodeCase(
                ...
                expected = LuaProofOutcome.ValidationFailure(41, 3, "entrypoint is not a function"),
            ),
            DecodeCase(
                ...
                expected = LuaProofOutcome.RuntimeFailure(41, 3, "callback failed"),
            ),
            DecodeCase(
                ...
                expected = LuaProofOutcome.MemoryFailure(41, 3, "allocator denied request", 256, 1024, 2, 64),
            ),
            DecodeCase(
                ...
                expected = LuaProofOutcome.Interrupted(41, 3, "instruction budget exceeded", 4567),
            ),
            DecodeCase(
                name = "cancelled operation",
                json = """{"kind":"cancelled","stateId":41,"generation":3,"operationId":29}""",
                expected = LuaProofOutcome.Cancelled(41, 3, 29),
            ),
            DecodeCase(
                ...
                expected = LuaProofOutcome.InvalidOwnership(41, 3, "operation belongs to state 42"),
            ),
            DecodeCase(
                name = "stale terminal completion diagnostic",
                json = """{"kind":"stale","stateId":41,"generation":3,"diagnostic":"operation already completed"}""",
                expected = LuaProofOutcome.Stale(41, 3, "operation already completed"),
            ),
            DecodeCase(
                name = "snapshot keeps Lua and bridge accounting separate",
                json = """{"kind":"completed","operation":"snapshot","stateId":41,"generation":3,"currentBytes":128,"peakBytes":1024,"deniedAllocations":2,"bridgeBytes":64,"elapsedNanos":4567,"luaVersion":"Lua 5.4.8","bindingVersion":"mlua 0.12.0","topology":"jvm_owned"}""",
                expected = LuaProofOutcome.Snapshot(41, 3, 128, 1024, 2, 64, 4567, "Lua 5.4.8", "mlua 0.12.0", "jvm_owned"),
            ),
            DecodeCase(
                name = "closed state",
                json = """{"kind":"closed","stateId":41,"generation":3}""",
                expected = LuaProofOutcome.Closed(41, 3),
            ),
        )

        cases.forEach { case ->
            assertEquals(case.name, case.expected, LuaProofOutcomeCodec.decode(case.json))
        }
    }
    ...
    fun `malformed unknown missing and type-invalid native outcomes normalize to runtime failure`() {
        ...
        cases.forEach { (name, json) ->
            val outcome = LuaProofOutcomeCodec.decode(json)
            assertTrue("$name must not escape JSON decoding: $outcome", outcome is LuaProofOutcome.RuntimeFailure)
        }
    }
