...
- [x] 3.2 Back up `release.keystore` to two physically separate encrypted offline locations (e.g., encrypted USB drive in a safe + attachment in a password manager). Record the backup locations in operator-private notes (not in the repo).
- [x] 3.3 Upload release keystore as base64: `base64 -w0 release.keystore | gh secret set ANDROID_RELEASE_KEYSTORE_BASE64`
- [x] 3.4 Upload keystore password: `gh secret set ANDROID_RELEASE_KEYSTORE_PASSWORD` (read from stdin via heredoc or `printf`)
- [x] 3.5 Upload key password: `gh secret set ANDROID_RELEASE_KEY_PASSWORD`
- [x] 3.6 Verify all three secrets exist: `gh secret list | grep ANDROID_RELEASE_`
- [x] 3.7 Delete the local `release.keystore` from disk only after backups and secrets are confirmed (the local copy can stay for testing but should not be relied upon)
...
- [x] 5.4 Verify locally that `nix develop --no-write-lock-file -c gradle assembleRelease` (without env) produces `app/build/outputs/apk/release/app-release-unsigned.apk` and does NOT fail
- [x] 5.5 Verify locally that `nix develop --no-write-lock-file -c gradle assembleRelease` with the three env vars set (pointing at a local copy of the release keystore) produces a signed `app-release.apk` and `apksigner verify --verbose app/build/outputs/apk/release/app-release.apk` succeeds
- [x] 5.6 Confirm `nix develop --no-write-lock-file -c gradle assembleDebug` still works unchanged
...
- [x] 7.7 Stage all PR-B files: `app/build.gradle.kts`, `.github/workflows/release.yml`, `.github/release-signing-pubkey.asc`, `RELEASE_SIGNING.md`, `README.md`, `.gitignore`

## 8. PR-B: review and merge

- [x] 8.1 Commit PR-B with message `feat: add release signing pipeline with GPG-signed tag verification`
- [x] 8.2 Push `release/signing-pipeline` and open PR titled "feat: release signing pipeline with GPG-signed tag verification"
- [x] 8.3 CI on PR-B runs only `ci.yml` (release.yml does not trigger on PRs by design); confirm `ci.yml` green on the PR
- [x] 8.4 Optional dry-run of release.yml on PR-B branch via `workflow_dispatch` (the workflow supports manual trigger): confirm "Verify release tag signature" step behaves correctly (it should fail on the untagged dispatch, confirming the verification is wired correctly)
- [x] 8.5 Merge PR-B to `main`

## 9. First release: end-to-end verification

- [x] 9.1 Edit `app/build.gradle.kts`: bump `versionCode` from `1` to `2`, bump `versionName` from `"0.1.0"` to `"0.2.0"`
- [x] 9.2 Commit on `main` with message `release: bump version to 0.2.0 for first signed release`
- [x] 9.3 Push and wait for `ci.yml` on `main` to report green
- [x] 9.4 Create signed tag: `git tag -s v0.2.0 -m "v0.2.0"` (uses Yubikey; expect touch prompt)
- [x] 9.5 Push the tag: `git push origin v0.2.0`
- [x] 9.6 Watch the Release workflow run: `gh run watch` — confirm every step green through "Publish GitHub Release"
- [x] 9.7 Verify the GitHub Release exists: `gh release view v0.2.0` — confirm APK asset is attached
- [x] 9.8 Verify the APK signature locally: `gh release download v0.2.0 --pattern '*.apk' --dir /tmp/release-check && nix develop -c apksigner verify --verbose /tmp/release-check/*.apk`
- [x] 9.9 Install on `B02PTT-FF01`: `nix develop -c adb install -r /tmp/release-check/*.apk` (note: this will fail if a debug build is already installed due to signature mismatch; uninstall first with `adb uninstall dev.nilp0inter.subspace` then install)
- [x] 9.10 Run the `AGENTS.md` manual acceptance flow on hardware: PTT press/release, Group → Control mode, Volume Up/Down in Control mode, echo with headset routing, foreground service notification on app switch, disconnect cleanup. All checks pass.
- [x] 9.11 Write the `v0.2.0` release notes manually (or edit the auto-generated ones) to include the signature-mismatch note for existing debug installs.

## 10. Post-merge housekeeping

- [x] 10.1 Update `AGENTS.md` "Standard Commands" or add a Releases reference pointing to `RELEASE_SIGNING.md` (operator-facing documentation)
- [x] 10.2 Verify Renovate picks up the new Actions pins in `release.yml` on its next run (the workflow uses the same pinning style as `ci.yml`)
- [x] 10.3 Optional: rotate the release-signing GPG key only if a stronger key algorithm or new Yubikey warrants it (otherwise leave as-is)
