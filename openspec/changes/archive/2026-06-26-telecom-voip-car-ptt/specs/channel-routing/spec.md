...
The system SHALL evaluate the target channel's readiness state before dispatching a PTT capture, regardless of whether PTT was started by the RSM, by phone channel-card long-press, or by Telecom-originated car PTT.
...
#### Scenario: Telecom car PTT on a ready active channel
- **WHEN** Telecom-originated car PTT is started and the active channel's `isReady` state is true
- **THEN** the system SHALL dispatch the capture to the active channel's designated controller
...
#### Scenario: Telecom car PTT on a not-ready active channel
- **WHEN** Telecom-originated car PTT is started and the active channel's `isReady` state is false
- **THEN** the system SHALL play a two-tone error beep when possible
- **AND** avoid placing or continue tearing down a Telecom capture connection
- **AND** drop the PTT capture without routing to a controller
