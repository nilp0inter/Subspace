...
Each log function SHALL return `(true, nil)` when the entry is recorded or silently rate-dropped within the bounded window, and `(nil, {error = "E_INVALID_VALUE"})` when the whole payload is rejected as invalid (contains non-serializable content). The host SHALL NOT recursively log the invalid payload, SHALL NOT strip or partially write fields, and SHALL NOT suspend, fail, or crash the calling coroutine when a payload is rejected.
...
- **AND** it SHALL NOT write a partial or incomplete log entry
...
- **WHEN** a callback issues log entries faster than the bounded rate limit allows
