...
- Fully relinquish exact-device car HFP priming before Telecom placement and require the active Bluetooth call route to exclude a readably identified target RSM and remain continuously acceptable before capture.
...
- `telecom-voip-car-ptt`: define normal car hang finalization, pre-capture and pre-response cleanup, exact-device HFP-to-Telecom handoff, stable active-route acceptance, and terminal Android Auto state/control/idle-retention behavior.
