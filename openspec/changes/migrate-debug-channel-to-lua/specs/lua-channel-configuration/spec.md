...
`ui` SHALL be an exact-key object containing only a `fields` array. Every data field ID SHALL appear exactly once in that array, whose order SHALL define render order. Each UI entry SHALL contain only canonical `field`, type-compatible `control`, nonblank bounded `label`, optional bounded `help`, and—only for `choice`—required `choices`. Controls SHALL be exactly `text` for unconstrained string, `toggle` for boolean, `number` for integer, or `choice` for a string with `allowedValues`. Each choice item SHALL contain exactly `value` and nonblank bounded `label`. The ordered set of choice values SHALL equal the data field's `allowedValues` set exactly, with no missing, extra, or duplicate value and no duplicate display label. Non-choice controls SHALL NOT declare `choices`. Empty UI fields SHALL match only empty data fields. Unknown keys at the `ui`, UI-entry, or choice-item level SHALL reject the package.

#### Scenario: Valid UI declaration matches data schema
- **WHEN** every data field has exactly one type-compatible UI entry and every choice value set exactly matches its `allowedValues`
...
#### Scenario: Choice declaration mismatches allowed values
- **WHEN** a choice targets a field without `allowedValues`, omits an allowed value, adds an extra value, duplicates a value, or duplicates a display label
- **THEN** validation SHALL reject the package before activation

#### Scenario: Unknown nested UI key
- **WHEN** `ui`, a UI field entry, or a choice item contains a key outside its exact v1 key set
