...
Defines the versioned, source-only Lua runtime boundary, module resolver, and
normalized host-operation contract.
...
- **WHEN** a callback returns a table that contains a cycle, has a metatable, or contains a function, userdata, thread, or platform value
...
Outside source-map module chunk evaluation, every public host operation available through the `subspace.*` modules SHALL return two values to the caller: `(value, nil)` on success and `(nil, error_table)` on failure, where `error_table` is a table with at least an `error` string field and optionally a `reason` field. During entry or lazy-module evaluation, the module-loader effect guard defined above takes precedence: a host-call attempt fails the whole load with the typed effect-call-during-load outcome and does not return a normal success/error pair to continued chunk evaluation. A yielding operation such as `subspace.runtime.sleep` SHALL suspend internally without exposing its operation token; a non-yielding admission operation such as `subspace.runtime.spawn` SHALL return synchronously. Cancellation of a live operation SHALL produce `(nil, {error = "E_CANCELLED"})`. The plugin SHALL NOT receive an opaque operation token, coroutine reference, userdata, or handle from any public host call. Duplicate completions, completions after cancellation, and completions after close SHALL be rejected by the host without resuming Lua.
...
The Lua function `coroutine.yield` SHALL be available for pure-Lua child-coroutine composition within a plugin's own Lua-space orchestration only. It SHALL NOT be treated as a host yielding operation and SHALL NOT suspend a spawned task across the actor boundary. A raw `coroutine.yield` call that escapes the active callback or spawned-task boundary without being composed via a child coroutine SHALL be recognized as an unrecognized yield and SHALL produce `E_INVALID_YIELD`, not a sleep or host operation. Event callbacks (`startup`, `handle_input`, `handle_readiness`, etc.) SHALL NOT call `sleep` or `coroutine.yield` at any stack depth.
...
#### Scenario: Event callback attempts to yield
- **WHEN** a synchronous event callback (`startup`, `handle_input`, `handle_readiness`, etc.) attempts to call `subspace.runtime.sleep` or `coroutine.yield` at any stack depth
- **THEN** the runtime SHALL treat the yield attempt as a callback contract violation
- **AND** it SHALL produce a typed callback-error outcome for that invocation
...
- **WHEN** `handle_input` or `handle_lifecycle` calls `subspace.runtime.spawn`
