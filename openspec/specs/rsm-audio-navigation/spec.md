...
The system SHALL play zero-latency audio announcements over the headset to confirm hardware-driven navigation actions, using a memoized TTS announcer that interrupts running playbacks. A traversal attempt beyond either catalogue boundary SHALL use the existing error beep through the same serialized SCO playback lifecycle.
...
- **AND** play the pre-computed name of the new active channel over the SCO headset route
...
- **AND** the system SHALL cancel any currently playing announcement
- **AND** play exactly one error beep over the SCO headset route
- **AND** release the acquired SCO route exactly once
...
- **THEN** the system SHALL cancel any currently playing announcement
- **AND** play a pre-computed confirmation phrase over the SCO headset route
