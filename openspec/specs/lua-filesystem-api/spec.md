...
Defines the bounded yielding `subspace.fs` document-tree API over mount handles: confined logical relative paths, directory creation and portable metadata, paginated listing, bounded complete UTF-8 reads and writes, explicit nonrecursive removal, a fixed portable failure model, and generation-safe exactly-once effects.
