...
The runtime SHALL own scheduling, mailbox admission, coroutine continuation, operation ownership, instruction and memory policy, failure containment, replacement draining, and deterministic teardown as internal mechanisms. Lua MAY eventually own protocol, retry, polling, and state-machine policy. Every blocking runtime or host operation SHALL yield an opaque operation token before execution and SHALL remain host-owned, lifecycle-bound, and generation-authorized. The runtime SHALL NOT define public Lua modules, a public Lua ABI, package format, installation or discovery flows, configuration schema, durable output or control API, official channel migrations, or plugin-driven foreground-service retention.

#### Scenario: Blocking host operation is requested
- **WHEN** a Lua callback needs a host effect that cannot complete in a deliberately small bounded native call
- **THEN** the runtime SHALL yield an opaque operation token before the effect executes
- **AND** the effect SHALL remain host-owned, lifecycle-bound, and generation-authorized
- **AND** the Lua execution thread SHALL NOT block waiting for external completion

#### Scenario: Plugin policy is expressed in Lua
- **WHEN** a future Lua-backed provider implements protocol, retry, polling, or state-machine policy
- **THEN** that policy SHALL execute within the actor's instruction and memory policy
- **AND** the runtime's scheduling, ownership, and containment mechanisms SHALL remain authoritative over that policy
