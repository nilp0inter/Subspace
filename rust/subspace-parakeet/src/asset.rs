...
//!
//! ONNX Runtime and `transcribe-rs` require filesystem paths, so Parakeet
//! model assets shipped in the APK `assets/` directory must be extracted to a
//! writable directory before the engine can load them. Extraction is
//! idempotent: a `.subspace_assets_version` marker file records the extracted
//! version. Already-up-to-date directories are skipped without re-copying.
//!
...
/// Expected Parakeet int8 model asset filenames, in the order they must be
/// extracted. The marker file is written only after every file in this list is
/// present and readable.
pub const PARAKEET_INT8_ASSETS: &[&str] = &[
...
/// - `version` is a caller-chosen tag (e.g. `"int8-2026-06-23"`). If the
///   existing marker matches, extraction is skipped.
/// - `assets` maps each filename in [`PARAKEET_INT8_ASSETS`] to its bytes.
pub fn extract_assets(
    ...
    for filename in PARAKEET_INT8_ASSETS {
        let bytes = assets
            .get(*filename)
...
mod tests {
    ...
    fn extracts_all_assets_and_writes_marker() {
        ...
        for name in PARAKEET_INT8_ASSETS {
    ...
    fn skips_when_marker_matches() {
        ...
        // Modify one file to detect re-extraction would happen if marker didn't match.
        std::fs::write(tmp.join("vocab.txt"), b"tampered").unwrap();
        let outcome = extract_assets(&tmp, "v1", &assets).unwrap();
        assert_eq!(outcome, ExtractionOutcome::AlreadyPresent);
        // Tampered file should remain because extraction was skipped.
        let contents = std::fs::read(tmp.join("vocab.txt")).unwrap();
    ...
    fn re_extracts_when_version_changes() {
        ...
        assert_eq!(outcome, ExtractionOutcome::Extracted);
        let contents = std::fs::read(tmp.join("vocab.txt")).unwrap();
        assert_eq!(contents, format!("contents-of-{}", "vocab.txt").as_bytes());
    }
    ...
    fn fails_when_asset_missing() {
        ...
        assets.remove("vocab.txt");
        let err = extract_assets(&tmp, "v1", &assets).unwrap_err();
        match err {
            ExtractionError::MissingAsset(name) => assert_eq!(name, "vocab.txt"),
